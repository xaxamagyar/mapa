...
# --- PREVENT NAME ERRORS BY INITIALIZING EMPTY DATA FRAMES BEFORE FETCHING ---
...
if not saved_routes:
    ...
    for r in reversed(saved_routes):
        with st.container():
            ...
            col_title.markdown(f"**🗓️ {r['name']}**<br>*(📦 {len(r.get('orders', []))} obj.)*", unsafe_allow_html=True)
            ...
            if os.path.exists(dr_path) and os.path.exists(di_path) and os.path.exists(wa_path) and os.path.exists(xl_path):
                ...
                col_dr.warning("⚠️ Otevřete a uložte rozvoz znovu (chybí cache).")
...
if not df_selected.empty:
    ...
    if st.button("🚀 Vypočítat časy a vygenerovat všechna PDF na obrazovku", type="primary"):
        ...
        # --- POMOCNÁ TŘÍDA A FUNKCE PRO PDF ---
        ...
        def build_page_one(pdf_obj, title_txt):
            ...
            pdf_obj.cell(0, 5, clean_str(f"Vygenerováno: {datetime.now().strftime('%d.%m.%Y %H:%M')} | Výjezd: {start_time.strftime('%H:%M')}"), ln=True, align="C")
            pdf_obj.ln(2)
            
            pdf_obj.set_font(font_family_name, "B", 16)
            pdf_obj.set_text_color(44, 62, 80)
            pdf_obj.cell(0, 10, clean_str(route_name_input), ln=True, align="C")
            pdf_obj.ln(2)
            ...
            pdf_obj.cell(95, 6, clean_str(f"Základní pokladna (Kasáč): {int(kasac_value)} Kč"), ln=True)
            pdf_obj.ln(6)
        ...
        pdf_driver.cell(0, 8, clean_str(f"ITINERÁŘ TRASY - {route_name_input}"), ln=True)
        pdf_driver.ln(2)
        ...
        pdf_disp.cell(0, 8, clean_str(f"ADMINISTRATIVNÍ PŘEHLED ZÁSILEK - {route_name_input}"), ln=True)
        pdf_disp.ln(2)
        ...
        pdf_ware.cell(0, 8, clean_str(f"POŘADÍ NAKLÁDKY A ZBOŽÍ - {route_name_input}"), ln=True)
        pdf_ware.ln(2)
...
if not df_selected.empty:
    ...
    if st.button("💾 ULOŽIT ROZVOZ DO HISTORIE (a vyčistit mapu)", type="primary", use_container_width=True):
        if not st.session_state.get('calc_main') or 'print_main' not in st.session_state:
            ...
            st.session_state['show_success_msg'] = "✅ Rozvoz byl bezpečně uložen do horní lišty a je připraven k tisku!"
            st.rerun()