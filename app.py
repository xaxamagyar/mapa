...
# --- CENTRÁLNÍ FUNKCE PRO GENEROVÁNÍ PDF ---
def generate_all_pdfs(route_name, df_itinerary, total_km, total_hours, total_cod, kasac_value, start_time_str, mapy_api_key):
    # Příprava fontů
    ...
    def generate_map_image(itinerary_df):
        ...
        for lat, lon in zip(lats, lons): 
            px, py = coord_to_px(lat, lon); pxs.append(px); pys.append(py)
        ...
        img_buf = io.BytesIO(); plt.savefig(img_buf, format='png', bbox_inches='tight', pad_inches=0.1)
    ...
    def build_page_one(pdf_obj, title_txt):
        ...
        pdf_obj.set_font(font_family_name, "", 9); pdf_obj.set_text_color(100, 100, 100)
        ...
        pdf_obj.set_font(font_family_name, "B", 10.5); pdf_obj.set_text_color(50, 50, 50)
        pdf_obj.set_fill_color(245, 246, 250); pdf_obj.set_draw_color(200, 200, 200)
        ...
        pdf_obj.cell(95, 6, clean_str(f"  Celkové dobírky: {int(total_cod)} Kč"), ln=False); pdf_obj.cell(95, 6, clean_str(f"Základní pokladna (Kasáč): {int(kasac_value)} Kč"), ln=True)
        pdf_obj.ln(6)
        if map_temp_img:
    ...
    for idx, row in df_itinerary.iterrows():
        ...
        if is_start or is_end:
            ...
            id_w = pdf_driver.get_string_width(id_str); p_name = prijemce
        ...
        note_raw = str(row.get('Poznámka', '')).strip(); has_note = bool(note_raw) and note_raw.lower() not in ['none', 'nan', '']
...
# --- SIDEBAR: NASTAVENÍ ČASŮ A API (Přesunuto z vrchu do postranního panelu) ---
...
# --- PŘEDNÍ PANEL: HISTORIE ULOŽENÝCH ROZVOZŮ (ON-THE-FLY TISK) ---
st.markdown("---")
...
if not saved_routes:
    ...
    for r in reversed(saved_routes):
        with st.container():
            col_title, col_dr, col_di, col_wa, col_up, col_del = st.columns([2.5, 1.5, 1.5, 1.5, 1.5, 1.5])
            col_title.markdown(f"**🗓️ {r['name']}**<br>*(📦 {len(r.get('orders', []))} obj. | {r.get('total_km', 0)} km | {r.get('total_cod', 0)} Kč)*", unsafe_allow_html=True)
            
            r_id = r.get('id', '')
            
            # Tlačítko pro generování PDF on-the-fly
            if col_dr.button("🖨️ Připravit PDF k tisku", key=f"prep_{r_id}", use_container_width=True):
                if 'itinerary_data' in r:
                    with st.spinner("Bleskově generuji PDF ze zálohy..."):
                        df_itin = pd.DataFrame(r['itinerary_data'])
                        pdf_dict = generate_all_pdfs(
                            ...
                            r.get('total_hours', ''), 
                            r.get('total_cod', 0), 
                            r.get('kasac_value', 2000), 
                            r.get('start_time_str', '06:00'), 
                            mapy_api_key
                        )
                        st.session_state[f"ready_pdfs_{r_id}"] = pdf_dict
                else:
                    st.error("Starý formát rozvozu. Prosím otevřete jej a uložte znovu.")
            ...
            # Pokud jsou PDF připravena, zobrazíme stahovací tlačítka v novém řádku
            if f"ready_pdfs_{r_id}" in st.session_state:
                pdf_dict = st.session_state[f"ready_pdfs_{r_id}"]
                st.success("✅ Soubory jsou připraveny ke stažení!")
                dl1, dl2, dl3 = st.columns(3)
                dl1.download_button("📥 Stáhnout ŘIDIČ", data=pdf_dict['pdf_dr'], file_name=f"{r['name']}_ridic.pdf", mime="application/pdf", key=f"dl_dr_{r_id}", type="primary", use_container_width=True)
                dl2.download_button("📥 Stáhnout DISPEČER", data=pdf_dict['pdf_di'], file_name=f"{r['name']}_dispecer.pdf", mime="application/pdf", key=f"dl_di_{r_id}", type="primary", use_container_width=True)
                dl3.download_button("📥 Stáhnout SKLAD", data=pdf_dict['pdf_wa'], file_name=f"{r['name']}_sklad.pdf", mime="application/pdf", key=f"dl_wa_{r_id}", type="primary", use_container_width=True)
                ...
                if s_lat and s_lon and e_lat and e_lon:
                    ...
                    if len(available_orders) < auto_min_orders: 
                        ...
                        if len(best_route_ids) >= auto_min_orders:
                            ...
                            st.success(f"Systém naplánoval {len(best_route_ids)} objednávek.")
                            time.sleep(2.5)
                            st.rerun()
                        else: 
                            st.error(f"Do nastavených limitů se nevešlo víc než {len(best_route_ids)} zastávek.")
                else: 
                    st.error("Nemohu najít souřadnice skladu.")
...
if not df_selected.empty:
                ...
                if start_lat is not None and start_lon is not None and end_lat is not None and end_lon is not None:
                    ...
                    for i in range(len(points)):
                        ...
                        for j in range(len(points)):
                            if i == j: 
                                dist_matrix[points[i]['id']][points[j]['id']] = 0.0
                            else: 
                                dist_matrix[points[i]['id']][points[j]['id']] = geodesic((points[i]['lat'], points[i]['lon']), (points[j]['lat'], points[j]['lon'])).kilometers
                    ...
                    st.error("Nepodařilo se zjistit souřadnice skladu.")
    ...
    if st.button("🚀 Vypočítat časy a vygenerovat všechny soubory", type="primary"):
        ...
        with st.spinner("Počítám časy přejezdů přes Mapy.cz..."):
            for i in range(len(df_itinerary) - 1):
                res_drive = get_driving_data(
                    df_itinerary.loc[i, 'lat'], df_itinerary.loc[i, 'lon'], 
                    df_itinerary.loc[i+1, 'lat'], df_itinerary.loc[i+1, 'lon'], mapy_api_key
                )
        ...
        with st.spinner("Vytvářím náhledová data..."):
            # Generujeme PDF ihned, abychom je mohli nabídnout hned pod mapou
            pdf_dict = generate_all_pdfs(
                route_name_input, 
                df_itinerary, 
                total_km, 
            ...
            st.session_state['print_main'] = {
                ...
                'cod': int(total_cod), 
                'df': df_final_display,
                'itinerary_data': df_itinerary.to_dict('records'),
                'pdf_dict': pdf_dict
            }
    ...
    if st.session_state.get('calc_main') and 'print_main' in st.session_state:
        ...
        # Místo přímého stažení tady nutíme uživatele Uložit rozvoz do historie.
